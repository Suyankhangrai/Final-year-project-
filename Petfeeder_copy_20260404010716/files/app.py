...
# Fixed: secret key from environment variable — never hardcode this.
...
# Fixed: removed hardcoded USER dict — all auth goes through the database only.
...
# Runs in a separate thread, checks every 30 seconds.
# If a schedule matches current time + day it queues the feed command.
# ================================================================
...
def login():
    if request.method == "POST":
        ...
        # Fixed: DB-only auth — hardcoded USER fallback removed
...
# Fixed: now uses db.get_pet_by_rfid() instead of raw SQL,
# and correct column name rfid_tag_uid instead of rfid_uid.
# Also removed non-existent daily_target column.
...
# Fixed: redirect was pointing to non-existent 'index' route.
...
def history():
    ...
    # Fixed: feeding_log uses 'source' not 'triggered_by'
    if mode != "all":
    ...
    if entries:
        for e in entries:
            # Fixed: feeding_log columns are 'source' and 'grams', not
            # 'triggered_by' and 'requested_grams'
            log_items += f"""
            <li class="log-item">
...
def add_schedule():
    ...
    # Fixed: cast to int so the DB receives a number, not a string
...
def settings():
    ...
    <div class="card">
      ...
      <div style="margin-top:12px;">
        <div class="pet-meta" style="margin-bottom:12px;"><strong style="color:var(--text-main);">Username:</strong> {session.get('user', 'Unknown')}</div>
...
# ================================================================
# ESP32 API — single, canonical versions of each endpoint
# ================================================================

@app.route("/device/online", methods=["GET"])
def device_online():
    """Heartbeat — ESP32 calls this every 60 s to stay marked online."""
...
def api_command():
    """ESP32 polls this every 5 s. Returns and clears any pending feed command."""
...
def api_pets():
    """Returns RFID + name + portion for all pets — used by the ESP32 on boot."""
    return jsonify(db.get_all_pets_api())

@app.route("/api/feed", methods=["POST"])
def api_feed():
    """
    ESP32 calls this after the motor finishes to log what was dispensed.
    Accepts JSON: { pet_name, grams, source }
    """
    try:
...
# ================================================================
# RUN — single call, at the very end of the file
# ================================================================
